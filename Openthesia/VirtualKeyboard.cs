    ...
    public static void ListenForKeyPresses()
    {
        foreach (var key in _keyNoteMap.Keys)
        {
            if (ImGui.IsKeyPressed(key, false))
            {
                ...
                Settings.ODevice.SendEvent(new NoteOnEvent(new SevenBitNumber((byte)(_keyNoteMap[key] + _octaveShift)), new SevenBitNumber((byte)_velocity)));
                _isKeyDown = true;
            }
            ...
            if (ImGui.IsKeyReleased(key))
            {
                ...
                Settings.ODevice.SendEvent(new NoteOffEvent(new SevenBitNumber((byte)(_keyNoteMap[key] + _octaveShift)), new SevenBitNumber(0)));
                _isKeyDown = false;
            }
        }
